...
public class FileRecv
{
        ...
        switch (tp)
        {
            case 11://开始上传
                try
                {
                    ...
                    //判断文件夹是否存在
                    string localpath = string.Format(@"{0}\{1}\{2}\{3}\", FilePath, request_file.CustomerID.ToString(), request_file.AtTime, request_file.WJID);
                ...
                {
                    ExitSelf(string.Format("[客户端id：{0},挖机号{1}]--该文件已近被占用", client.CustomerID, client.code));
                }
                break;
            case 12://上传中
                ...
                {
                    ExitSelf(string.Format("[客户端id：{0},挖机号{1}]--上传中错误退出--{2}", client.CustomerID, client.code,ex.ToString()));
                }
                break;
            case 13://完成
                try
    ...
    private void ExitSelf(string msg)
    {
        Exit();
        Debug.Error(msg);
        client.close();
